...
def im_detect(sess, net, image_id, Test_RCNN, keys, scores_P, scores_A, scores_L, bboxes, hdet, odet, pointers):
    This_image   = []
        ...
        if Human_out[2] == 1:
            ...
            if blobs['H_num'] > 0:
                ...
                for j in range(blobs['H_num']):
                    ...
                    score_P = P[j][obj_range[classid][0] - 1:obj_range[classid][1]].reshape(1, -1)
                    if scores_P[classid] is None:
                      P_range = score_P.shape[1]
                      scores_P[classid] = np.empty((cfg.MAX_POINTER, P_range),dtype=np.float64)
                    scores_P[classid][pointers[0][classid]] = score_P
                    pointers[0][classid] += 1                    
                    
                    score_A = A[j][obj_range[classid][0] - 1:obj_range[classid][1]].reshape(1, -1)
                    if scores_A[classid] is None:
                      A_range = score_A.shape[1]
                      scores_A[classid] = np.empty((cfg.MAX_POINTER, A_range),dtype=np.float64)
                    scores_A[classid][pointers[1][classid]] = score_A
                    pointers[1][classid] += 1
                    
                    score_L = L[j][obj_range[classid][0] - 1:obj_range[classid][1]].reshape(1, -1)
                    if scores_L[classid] is None:
                      L_range = score_L.shape[1]
                      scores_L[classid] = np.empty((cfg.MAX_POINTER, L_range),dtype=np.float64)
                    scores_L[classid][pointers[2][classid]] = score_L
                    pointers[2][classid] += 1
                    
                    
                    hbox = np.array(Human_out[1]).reshape(1, -1)
                    obox = np.array(Object[1]).reshape(1, -1)
                    
                    bbox = np.concatenate([hbox, obox], axis=1)
                    if bboxes[classid] is None:
                      bbox_range = bbox.shape[1]
                      bboxes[classid] = np.empty((cfg.MAX_POINTER, bbox_range),dtype=np.float64)
                    bboxes[classid][pointers[3][classid]] = bbox
                    pointers[3][classid] += 1
                    ...
                    # print('***********')
                    # print(P[j][obj_range[classid][0] - 1:obj_range[classid][1]].reshape(1, -1).shape)
                    # print(A[j][obj_range[classid][0] - 1:obj_range[classid][1]].reshape(1, -1).shape)
                    # print(L[j][obj_range[classid][0] - 1:obj_range[classid][1]].reshape(1, -1).shape)
                    # print(np.concatenate([hbox, obox], axis=1).shape)
                    # print(Human_out[3])
                    # print(Object[3])
                    # print('***********')
    # del im, im_orig, blobs, Human, Object, hbox, obox, classid, P, A, L, index, H_box, P_box, Human_out

def test_net(sess, net, Test_RCNN, output_dir, test_list_used):
    np.random.seed(cfg.RNG_SEED)
    detection = {}
    count = 0
    _t = {'im_detect' : Timer(), 'misc' : Timer()}
    pointers = np.zeros((4,80), np.int32)
    keys, scores_P, scores_A, scores_L, bboxes, hdet, odet = [], [], [], [], [], [], []
    for i in range(80):
        keys.append([])
        scores_P.append(None)
        scores_A.append(None)
        scores_L.append(None)
        bboxes.append(None)
        hdet.append([])
        odet.append([])

    for line in test_list_used:#glob.iglob(cfg.DATA_DIR + '/' + 'hico_20160224_det/images/test2015/*.jpg'):
        _t['im_detect'].tic()
 
        image_id   = int(line[-9:-4])
        
        im_detect(sess, net, image_id, Test_RCNN, keys, scores_P, scores_A, scores_L, bboxes, hdet, odet, pointers)
        
        _t['im_detect'].toc()

        # print('im_detect: {:d}/{:d} {:.3f}s'.format(count + 1, 9658, _t['im_detect'].average_time))
        print('im_detect: {:d}/{:d} {:.3f}s'.format(count + 1, len(test_list_used), _t['im_detect'].average_time))
        count += 1
    
    for i in range(80):
        scores_P[i] = scores_P[i][:pointers[0][i]]
        scores_A[i] = scores_A[i][:pointers[1][i]]
        scores_L[i] = scores_L[i][:pointers[2][i]]
        bboxes[i]   = bboxes[i][:pointers[3][i]]
    ...
    joblib.dump(scores_P, os.path.join(output_dir, 'scores_P.pkl'))
    joblib.dump(scores_A, os.path.join(output_dir, 'scores_A.pkl'))
    joblib.dump(scores_L, os.path.join(output_dir, 'scores_L.pkl'))
    joblib.dump(bboxes, os.path.join(output_dir, 'bboxes.pkl'))
    joblib.dump(keys, os.path.join(output_dir, 'keys.pkl'))
    joblib.dump(hdet, os.path.join(output_dir, 'hdet.pkl'))
    joblib.dump(odet, os.path.join(output_dir, 'odet.pkl'))
